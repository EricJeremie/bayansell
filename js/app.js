...
/* Global fallback image (gray placeholder) — global so inline
   onerror handlers in rendered HTML can reference it. */
window.FALLBACK_IMG =
  "data:image/svg+xml," +
  encodeURIComponent(
...
(function () {
  ...
  var HEART_SVG =
    '<svg viewBox="0 0 32 32" aria-hidden="true"><path d="M16 28c7-4.7 12-9.1 12-15 0-3.3-2.7-6-6-6-2.4 0-4.6 1.4-6 3.6C14.6 8.4 12.4 7 10 7c-3.3 0-6 2.7-6 6 0 5.9 5 10.3 12 15z"/></svg>';

  var BACK_SVG =
    '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.4 7.4 14 6l-6 6 6 6 1.4-1.4-4.6-4.6z"/></svg>';

  // Simple line-icon glyphs (inner SVG) for the Airbnb-style category row.
  var CATEGORY_ICONS = {
  ...
  function iconSvg(name) {
    return (
      ...
      (CATEGORY_ICONS[name] || CATEGORY_ICONS["All"]) +
      "</svg>"
    );
  }
  ...
  function cardHTML(l) {
    var fav = Store.isFavorite(l.id) ? " is-fav" : "";
    ...
    return (
      ...
      '<span class="card-condition">' + esc(l.condition) + "</span>" +
      "</div>" +
  ...
  function categoryRowHTML() {
    ...
    return (
      '<div class="filters"><div class="category-tabs">' +
      cats
        .map(function (c) {
          return (
  ...
  function toolbarHTML(count) {
    ...
    return (
      ...
      (state.search ? ' for “' + esc(state.search) + "”" : "") +
      "</div>" +
      '<div class="sort-control"><label for="sort-select">Sort by</label>' +
  ...
  function renderBrowse() {
    ...
    var hero =
      '<section class="hero"><h1>Find your next great deal</h1>' +
      "<p>Buy and sell preloved and brand-new items with people across the Philippines.</p></section>";
    ...
    if (list.length === 0) {
      body =
        ...
        '<button class="btn btn-secondary" data-action="clear">Clear filters</button></div>';
    } else {
      body = toolbarHTML(list.length) + gridHTML(list);
    }
  ...
  function galleryHTML(images) {
    ...
    if (imgs.length === 1) {
      return (
        '<div class="gallery single"><div class="gallery-main">' +
        imgTag(imgs[0], "") +
        "</div></div>"
      );
    }
    var side = imgs
      .slice(1, 3)
      .map(function (src) {
        return '<div class="gallery-cell">' + imgTag(src, "") + "</div>";
      })
      .join("");
    return (
  ...
  function specHTML(label, value) {
    return (
      '<div class="spec"><span class="spec-label">' +
      esc(label) +
  ...
  function renderDetail(id) {
    var l = Store.getById(id);
    if (!l) {
      app.innerHTML =
        '<div class="empty"><div class="emoji">🤔</div><h2>Listing not found</h2>' +
        "<p>This item may have been removed or the link is incorrect.</p>" +
        '<a class="btn btn-primary" href="#/">Back to browse</a></div>';
      return;
    }
    ...
    app.innerHTML =
      '<div class="detail">' +
      '<button class="back-link" type="button" data-action="back">' + BACK_SVG + " Back to results</button>" +
      '<div class="detail-head"><div>' +
      '<h1 class="detail-title">' + esc(l.title) + "</h1>" +
      '<div class="detail-sub">' + esc(l.category) + " · " + esc(l.location) +
      " · Posted " + formatDateRel(l.postedAt) + "</div></div>" +
      '<button class="detail-save' + favCls + '" type="button" data-fav="' + esc(l.id) + '">' +
      HEART_SVG + "<span>" + (isFav ? "Saved" : "Save") + "</span></button>" +
      "</div>" +
      galleryHTML(l.images) +
      '<div class="detail-grid"><div class="detail-left">' +
      '<section class="detail-section"><h2>Description</h2>' +
      '<div class="detail-desc">' + esc(l.description) + "</div></section>" +
      '<section class="detail-section"><h2>Item details</h2><div class="spec-list">' +
      specHTML("Condition", l.condition) +
      specHTML("Category", l.category) +
      specHTML("Location", l.location) +
      specHTML("Posted", formatDateAbs(l.postedAt)) +
      "</div></section>" +
      '<section class="detail-section"><h2>Seller</h2><div class="seller-row">' +
      '<div class="seller-avatar">' + esc(initials(l.seller.name)) + "</div>" +
      "<div><div class=\"seller-name\">" + esc(l.seller.name) + "</div>" +
      '<div class="seller-meta">' + esc(sellerMeta) + "</div></div></div></section>" +
      "</div>" +
      '<aside class="detail-right"><div class="price-card">' +
      '<div class="price">' + formatPrice(l.price) + " <small>· " + esc(l.condition) + "</small></div>" +
      '<span class="condition-tag">' + esc(l.condition) + "</span>" +
      '<div class="price-meta">📍 ' + esc(l.location) + " · Posted " + formatDateRel(l.postedAt) + "</div>" +
  ...
  function renderPreviews() {
    ...
    box.innerHTML = pendingImages
      .map(function (src, i) {
        return (
          '<div class="thumb">' +
          imgTag(src, "Photo " + (i + 1)) +
          '<button type="button" data-remove-img="' + i + '" aria-label="Remove photo">✕</button>' +
          "</div>"
        );
      })
      .join("");
  ...
  function handleFiles(fileList) {
    ...
    Promise.all(files.map(resizeImage))
      ...
      .catch(function () {
        toast("Sorry, one of those images couldn’t be read.");
      });
  }
  ...
  function submitPost(form) {
    ...
    var ok = true;
    setError("title", data.title ? "" : "Please add a title.");
    if (!data.title) ok = false;
    setError("category", data.category ? "" : "Please choose a category.");
    if (!data.category) ok = false;
    var priceNum = Number(data.price);
    var priceMsg = !data.price ? "Please set a price." : priceNum <= 0 ? "Price must be greater than 0." : "";
    setError("price", priceMsg);
    if (priceMsg) ok = false;
    setError("condition", data.condition ? "" : "Please choose a condition.");
    if (!data.condition) ok = false;
    setError("location", data.location ? "" : "Please choose a location.");
    if (!data.location) ok = false;
    setError("description", data.description ? "" : "Please add a short description.");
    if (!data.description) ok = false;
  ...
  function renderFavorites() {
    var list = Store.getFavoriteListings();
    if (list.length === 0) {
      app.innerHTML =
        '<section class="hero"><h1>Your saved items</h1></section>' +
        '<div class="empty"><div class="emoji">❤️</div>' +
        "<h2>No saved items yet</h2>" +
        "<p>Tap the heart on any listing to save it here for later.</p>" +
        '<a class="btn btn-primary" href="#/">Start browsing</a></div>';
      return;
    }
    app.innerHTML =
      '<section class="hero"><h1>Your saved items</h1>' +
      "<p>" + (list.length === 1 ? "1 item" : list.length + " items") + " you’ve saved for later.</p></section>" +
      gridHTML(list);
  }
  ...
  function openAuthModal(afterAuth) {
    var overlay = openModal(
      "Log in or sign up",
      "<h2>Welcome to Bayansell</h2>" +
        '<p class="modal-sub">Log in or sign up to sell your items and message sellers.</p>' +
        '<form id="auth-form" novalidate>' +
        '<div class="field"><label for="auth-name">Name</label>' +
        '<input class="input" id="auth-name" type="text" placeholder="e.g. Juan dela Cruz" />' +
        '<div class="error-text" data-err="auth-name" hidden></div></div>' +
    ...
    setTimeout(function () {
      var n = document.getElementById("auth-name");
      if (n) n.focus();
    }, 60);
  }
  ...
  function accountMenuHTML() {
    ...
    var savedItem =
      '<button class="menu-item" data-nav="#/favorites">Saved' +
    ...
    if (user) {
      ...
      return (
        ...
        '<div class="menu-sep"></div>' +
        '<button class="menu-item" data-action="logout">Log out</button>'
      );
    }
    return (
      '<button class="menu-item bold" data-action="login">Log in</button>' +
      '<button class="menu-item" data-action="login">Sign up</button>' +
      '<div class="menu-sep"></div>' +
      '<button class="menu-item" data-nav="#/post">Sell your item</button>' +
      savedItem
    );
  ...
  function renderMyListings() {
    ...
    if (!user) {
      app.innerHTML =
        '<div class="empty"><div class="emoji">🔑</div><h2>Log in to see your listings</h2>' +
        "<p>Sign in to view and manage the items you’ve posted.</p>" +
        '<button class="btn btn-primary" type="button" data-action="login-inline">Log in or sign up</button></div>';
      return;
    }
    if (user.role !== "seller") {
    ...
    if (mine.length === 0) {
      app.innerHTML =
        '<section class="hero"><h1>Your listings</h1></section>' +
        '<div class="empty"><div class="emoji">📦</div><h2>No listings yet</h2>' +
        "<p>Items you post will show up here, ready to manage.</p>" +
        '<a class="btn btn-primary" href="#/post">Sell your item</a></div>';
      return;
    }
    var grid = mine
      .map(function (l) {
      ...
      .join("");
    app.innerHTML =
      '<section class="hero"><h1>Your listings</h1>' +
      "<p>" + (mine.length === 1 ? "1 item" : mine.length + " items") + " you’ve posted on Bayansell.</p></section>" +
  ...
  function confirmDelete(id) {
    ...
    var overlay = openModal(
      ...
      '<h2>Delete this listing?</h2>' +
        '<p class="modal-sub">“' + esc(l ? l.title : "") + '” will be permanently removed. This can’t be undone.</p>' +
        '<div style="display:flex;gap:10px;justify-content:flex-end">' +
        '<button class="btn btn-secondary" type="button" data-action="close-modal">Cancel</button>' +
        '<button class="btn btn-primary" type="button" id="confirm-delete">Delete</button></div>'
    );
    overlay.querySelector("#confirm-delete").addEventListener("click", function () {
      Store.deleteListing(id);
      closeModal();
      toast("Listing deleted");
      if (parseHash().name === "item") location.hash = "#/my-listings";
      else { render(); }
    });
  }
  ...
  function openContactModal(id) {
    ...
    var overlay = openModal(
      "Contact seller",
      "<h2>Message " + esc(l.seller.name) + "</h2>" +
        '<p class="modal-sub">Ask about this item. This is a demo — nothing is actually sent.</p>' +
        '<div class="modal-summary"><img src="' + esc(cover) + '" onerror="this.onerror=null;this.src=FALLBACK_IMG" alt="">' +
        '<div><div class="ms-title">' + esc(l.title) + '</div>' +
        '<div class="ms-price">' + formatPrice(l.price) + " · " + esc(l.location) + "</div></div></div>" +
        '<form id="contact-form" novalidate>' +
        '<div class="field"><label for="c-name">Your name</label>' +
        '<input class="input" id="c-name" type="text" value="' + esc(user ? user.name : "") + '" placeholder="Your name" />' +
        '<div class="error-text" data-err="c-name" hidden></div></div>' +
        '<div class="field"><label for="c-message">Message</label>' +
        '<textarea class="textarea" id="c-message" placeholder="Hi! Is this still available?">Hi! Is this still available?</textarea>' +
        '<div class="error-text" data-err="c-message" hidden></div></div>' +
        '<button class="btn btn-primary btn-block" type="submit">Send message</button>' +
        "</form>"
    );
    overlay.querySelector("#contact-form").addEventListener("submit", function (e) {
      e.preventDefault();
      var name = document.getElementById("c-name").value.trim();
      var msg = document.getElementById("c-message").value.trim();
      var ok = true;
      setFieldError("c-name", name ? "" : "Please enter your name."); if (!name) ok = false;
      setFieldError("c-message", msg ? "" : "Please enter a message."); if (!msg) ok = false;
      if (!ok) return;
      closeModal();
      toast("Message sent to " + l.seller.name.split(" ")[0] + "! ✉️");
    });
  }
  ...
  function updateHeaderActive(route) {
    var saved = document.getElementById("saved-link");
    if (saved) saved.classList.toggle("active", route.name === "favorites");
  }

  function updateSavedBadge() {
    var badge = document.getElementById("saved-count");
    if (!badge) return;
    var n = Store.favoriteCount();
    badge.textContent = n;
    badge.hidden = n === 0;
  }
  ...
  function handleFav(id) {
    ...
    render();
    window.scrollTo(0, y);
    toast(Store.isFavorite(id) ? "Saved to your favorites ♥" : "Removed from favorites");
  }
  ...
  function bindEvents() {
    ...
    app.addEventListener("click", function (e) {
      ...
      if (favBtn) {
        e.preventDefault();
        handleFav(favBtn.getAttribute("data-fav"));
        return;
      }
      ...
      if (contact) { e.preventDefault(); openContactModal(contact.getAttribute("data-id")); return; }

      var del = e.target.closest("[data-delete]");
      if (del) { e.preventDefault(); confirmDelete(del.getAttribute("data-delete")); return; }
    ...
    // Drag & drop onto the photo dropzone.
    ...
    if (searchInput) {
      searchInput.addEventListener("input", function () {
        ...
        if (parseHash().name !== "browse") location.hash = "#/";
        else render();
      });
    }
    if (searchForm) {
    ...
    if (accountMenu) {
      accountMenu.addEventListener("click", function (e) {
        ...
        if (nav) { toggleAccountMenu(false); location.hash = nav.getAttribute("data-nav"); return; }
    ...
    // Close the menu on outside click; close menu/modal on Escape.
    document.addEventListener("click", function (e) {
      var menu = document.getElementById("account-menu");
      if (menu && !menu.hidden && !e.target.closest(".account-wrap")) toggleAccountMenu(false);
    });
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape") { closeModal(); toggleAccountMenu(false); }
    });
