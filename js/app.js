...
(function () {
  ...
  async function renderPost(editId) {
    ...
    app.innerHTML = '<div class="form-page"><h1>' + title + '</h1><p class="lead">List your item in a couple of minutes. Add clear photos and an honest description to sell faster.</p><form id="post-form" novalidate data-edit-id="' + (editId || "") + '">' +
      ...
      '<div class="field-row"><div class="field"><label for="pf-category">Category</label><select class="select" id="pf-category">' + selectOptions(CATEGORIES, "Select a category", l ? l.category : null) + '</select><div class="error-text" data-err="category" hidden></div></div><div class="field"><label for="pf-condition">Condition</label><select class="select" id="pf-condition">' + selectOptions(CONDITIONS, "Select condition", l ? l.condition : null) + '</select><div class="error-text" data-err="condition" hidden></div></div></div>' +
      '<div class="field-row"><div class="field"><label for="pf-price">Price</label><div class="price-input"><span class="peso">₱</span><input class="input" id="pf-price" type="number" min="1" step="1" value="' + (l ? l.price : "") + '" placeholder="0" /></div><div class="error-text" data-err="price" hidden></div></div><div class="field"><label for="pf-location">Location</label><select class="select" id="pf-location">' + selectOptions(PH_CITIES, "Select a city", l ? l.location : null) + '</select><div class="error-text" data-err="location" hidden></div></div></div>' +
  ...
  async function accountMenuHTML() {
    var user = await Store.getUser();
    var favCount = await Store.favoriteCount();
  ...
  async function renderDashboard() {
    ...
    var listings = await Store.getUserListings();
    ...
    var inquiries = await Store.getInquiries();
  ...
  async function confirmDelete(id) {
    ...
    overlay.querySelector("#confirm-delete").addEventListener("click", async function () { await Store.deleteListing(id); closeModal(); toast("Listing deleted"); if (parseHash().name === "item") location.hash = "#/"; else { await render(); } });
  }

  async function openContactModal(id) {
    var l = await Store.getById(id); if (!l) return;
    var user = await Store.getUser();
  ...
  function bindEvents() {
    app.addEventListener("click", async function (e) {
      ...
      if (upgrade) { e.preventDefault(); await Store.switchRole("seller"); toast("You are now a Seller! 💼"); await render(); return; }
    ...
    app.addEventListener("submit", async function (e) {
      ...
      else if (e.target.id === "chat-form") { e.preventDefault(); var inqId = state.activeInquiryId; var input = document.getElementById('chat-input'); var text = input ? input.value.trim() : ""; if (!text || !inqId) return; await Store.sendMessage(inqId, text); await render(); }
    });
    ...
    if (accountMenu) {
      accountMenu.addEventListener("click", async function (e) {
        ...
        if (act) {
          ...
          else if (a === "logout") { await toggleAccountMenu(false); await Store.logout(); await updateAuthUI(); toast("You’ve been logged out"); if (parseHash().name === "dashboard") location.hash = "#/"; else await render(); }
          else if (a === "toggle-role") { var next = act.getAttribute("data-role"); await Store.switchRole(next); await toggleAccountMenu(false); toast("Account switched to " + (next === "seller" ? "Seller" : "Buyer")); if (next === "buyer" && parseHash().name === "dashboard") location.hash = "#/"; else await render(); }
        }
      });
    }
